...
- **主干教材**：Kurose & Ross《计算机网络：自顶向下方法》第 8 版——提供 9 阶段的自顶向下骨架，30 个课时中 29 课标注了对应章节；
- **事实基准**：16 份 RFC 原文，另含 IEEE 802.11 / 802.1Q、NIST SP 800-115、等保 2.0 与《TCP/IP 详解 卷1》——所有协议细节以一手标准为准，教材引用经制作期审计与溯源清单编制时的官方目录全量复核（见 CHANGELOG 0.5 与相应版本条目）；
- **实验体系**：Kurose 官方 Wireshark Labs 与 Cisco Packet Tracer——实验数据取自官方脱敏 trace，不碰真实流量。

蒸馏方法：教材定框架 → 标准定事实 → 官方实验定动手环节 → 按私教课结构（问题驱动、7 节、阶段项目）重组。**完整的 30 课时逐课溯源映射见 [docs/sources.md](docs/sources.md)。**
...
```
├── index.html                  # 课程本体（当前版本 v1.4）
├── CHANGELOG.md                # 修复日志：0.x 开发史 + v1.0–v1.4 明细 + 人工检查指南
├── docs/
│   ├── course-plan.md          # 课程计划（9 阶段 30 课时 + 2.6 图表/质量规范）
│   ├── sources.md              # 知识来源与蒸馏溯源清单（30 课时逐课映射）
...
4. 想看知识来源？[docs/sources.md](docs/sources.md) 有 30 课时的逐课溯源表——每课标注了教材章节、RFC 与官方实验。
